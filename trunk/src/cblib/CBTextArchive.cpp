...
void CBTextArchive::Initialize(wxInputStream &input)
{
	...
	for(size_t i=0;i<size;i++){
		//posizione della sezione di testo corrente
...
{
	wxUint32* pointersBuf=new wxUint32[size]; //buffer dei puntatori ai blocchi di testo
	wxUint32* numPointersBuf=new wxUint32[size]; //buffer della seconda parte di header
